import json, os, joblib, pandas as pd, numpy as np
...
from module_fluid_engineering.model_layer.training.build_supervised import build_supervised
from module_fluid_engineering.model_layer.training.rollout_training import generate_rollout_dataset
...
def train_model_v3(measurements, fluids_meta, n_rounds=3):
    ...
    # -----------------------------------------------------
    X, y = build_supervised(measurements, fluids_meta)
    ...
    params_path = os.path.join(CONFIG_DIR, "best_params.json")
    with open(params_path, "w") as f:
        json.dump(best_params, f, indent=4)

    # 🔹 2. Salvar resultados completos da busca
    cv_results = pd.DataFrame(search.cv_results_)
    cv_path = os.path.join(CONFIG_DIR, "cv_results.csv")
    ...
    for k in range(n_rounds):
        ...
        Xr, yr = generate_rollout_dataset(
            model,
            measurements,
            fluids_meta,
            p_use_pred=p
    ...
    features_path = os.path.join(CONFIG_DIR, f"features.json")
